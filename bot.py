...
def start_keyboard():
    ...
    if not TEST_MODE:
        keyboard.append([{ "text": f"⭐ Оплатить {PRICE_STARS} Stars", "callback_data": "pay_quiz" }])
...
def send_start(chat_id):
    ...
    tg("sendMessage", {"chat_id": chat_id, "text": text, "parse_mode": "HTML", "reply_markup": start_keyboard()})
...
def find_book(title):
    ...
    for pdf in pdfs:
        if normalize(pdf.stem) == wanted:
            return pdf
    for pdf in pdfs:
        if wanted and wanted in normalize(pdf.stem):
            return pdf
...
def send_book(chat_id, title):
    ...
    if not pdf:
        ...
        tg("sendMessage", {"chat_id": chat_id, "text": f"📕 Твоя книга: <b>{title}</b>\n\nPDF этой книги пока не подключён к каталогу.", "parse_mode": "HTML"})
        return False
...
def handle_message(message):
    ...
    if message.get("successful_payment"):
        ...
        tg("sendMessage", {"chat_id": chat_id, "text": "✅ <b>Оплата прошла!</b>\n\nДоступ открыт. Нажми «🎲 Пройти тест».", "parse_mode": "HTML", "reply_markup": start_keyboard()})
        return
...
def health():
    ...
    return jsonify({"ok": True, "service": "book-quiz-bot", "test_mode": TEST_MODE, "webhook_url": WEBHOOK_URL, "webhook_info": webhook_info, "web_app_url": WEB_APP_URL, "books_dir": str(BOOKS_DIR), "pdf_count": len(list(BOOKS_DIR.rglob("*.pdf"))) if BOOKS_DIR.exists() else 0})
